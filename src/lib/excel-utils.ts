...
export async function exportInvoiceExcel(
  ...
  if (invoice.is_potong_material === 1) {
    mainAoa.push(['TOTAL HARGA KOTOR', '', '', '', `${trips.length} Rit`, invoice.total_kubikasi, '', invoice.total_harga_kotor]);
    ...
    mainAoa.push(['TOTAL NILAI PO', '', '', '', `${trips.length} Rit`, invoice.total_kubikasi, '', invoice.total_harga_kotor]);
  }
